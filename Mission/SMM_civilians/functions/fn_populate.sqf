...
// Spawn pedestrians
[_pedCount,FUNC(spawnPedestrian),[createGroup [civilian,true],_area,_blacklist,_unitClasses,_customInit,_customArgs,_ambCiv],GVAR(pedSpawnDelay),true] call EFUNC(common,iterate);

// Spawn moving vehicles/drivers
[_driverCount,FUNC(spawnDriver),[_area,_blacklist,_unitClasses,_vehicleClasses,_customInit,_customArgs,_ambCiv],GVAR(driverSpawnDelay),true] call EFUNC(common,iterate);

// Spawn parked vehicles
[_parkedCount,FUNC(spawnParked),[_area,_blacklist,_vehicleClasses,_customInit,_customArgs,_ambCiv],GVAR(parkedSpawnDelay),true] call EFUNC(common,iterate);
