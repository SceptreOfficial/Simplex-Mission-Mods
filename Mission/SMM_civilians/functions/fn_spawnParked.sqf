...
while {
	...
	private _road = [_randPos,_roads] call TRIPLES(DOUBLES(SMM,common),fnc,getNearest);
	...
	if !(_buildings isEqualTo []) then {
		...
		if (!(_spawnPos isEqualTo []) && {isOnRoad _spawnPos}) then {
			private _road = [_spawnPos,_spawnPos nearRoads 15] call TRIPLES(DOUBLES(SMM,common),fnc,getNearest);
